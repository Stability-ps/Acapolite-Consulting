-- Prerequisite for bulk client import: a client record must be able to
-- exist without a linked portal account (profile_id). Previously
-- clients.profile_id was required, so every client had to go through
-- signup/invite before a staff member could even create a record for them.
--
-- Also fixes a duplicate-client-creation bug in handle_new_user(): when a
-- client accepted a portal invite tied to an existing clients row
-- (raw_user_meta_data.linking_client_id), the trigger was unconditionally
-- inserting a *new* clients row instead of linking to the existing one,
-- leaving two records for the same person. The insert now branches on
-- linking_client_id, and the profile_id-keyed upsert uses
-- "on conflict (profile_id) do update" instead of a plain insert.
--
...
begin
  ...
  v_linking_client_id := nullif(trim(coalesce(new.raw_user_meta_data ->> 'linking_client_id', '')), '')::uuid;
  ...
  if v_role = 'client' then
    if v_linking_client_id is not null then
      update public.clients
      set profile_id = new.id,
          updated_at = now()
      where id = v_linking_client_id
