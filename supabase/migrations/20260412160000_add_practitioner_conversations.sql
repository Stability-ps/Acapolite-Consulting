alter table public.conversations
  add column if not exists practitioner_profile_id uuid references public.profiles(id) on delete set null;
...
begin
  if not exists (
    ...
    where constraint_schema = 'public'
      ...
      and constraint_name = 'conversations_participant_chk'
  ) then
    alter table public.conversations
      add constraint conversations_participant_chk
      check (client_id is not null or practitioner_profile_id is not null);
  end if;
end $$;

create index if not exists idx_conversations_practitioner_profile_id
  on public.conversations(practitioner_profile_id);
