...
type FakeMessageRow = { id: string; conversation_id: string; delivery_status: string | null };

// A minimal stand-in for the Supabase client used only by applyStatusUpdate:
// `.from("whatsapp_messages").select(...).eq(...).maybeSingle()`,
// `.from("whatsapp_messages").update(...).eq(...)`, and
// `.from("whatsapp_alerts").insert(...)`. Records every call so tests can
...
Deno.test("applyStatusUpdate records a failed status with a useful, short error detail", async () => {
  const { client, calls } = fakeClient({ message: { id: "msg-1", conversation_id: "conv-1", delivery_status: "sent" } });
...
Deno.test("applyStatusUpdate ignores a status for a meta_message_id Acapolite never sent", async () => {
