// Platform-variant generation, extracted from social-generate-variants/index.ts
// so it can be unit tested against a fake store instead of a live Supabase
// project. The historical bug this guards against: a single-poster
// "Generate platform versions" click regenerating variants for OTHER
// posters too. The fix is structural, not just a request-shape check -
// generateVariantsForAsset takes exactly one assetId and every dependency
// call it makes is scoped to that id, so there is no code path left that
// could touch a second asset's rows or storage objects.
import { platformKeyForAccountPlatform, validateAssetForPlatform } from "./socialPlatformRules.ts";
import { chooseInstagramFeedTarget, computeContainLayout, FACEBOOK_FALLBACK_TARGET, needsManualAdjustment } from "./socialImageTransform.ts";
import { generateContainVariant, sha256Hex } from "./socialImageProcessor.ts";
...
export async function generateVariantsForAsset(deps: GenerateVariantsDeps, assetId: string): Promise<VariantResult[]> {
  ...
  for (const platform of VARIANT_PLATFORMS) {
    ...
    await deps.logActivity("social_variant_generated", asset.id, { platform, width: generated.width, height: generated.height, regenerated: Boolean(existingVariant) });
