...
// separate from the actual pixel manipulation (socialImageProcessor.ts,
...
// logos near the edges of a poster, which the product requirement
// explicitly forbids doing silently. Contain can never lose content; the
// only cost is padding. When contain would need to pad away more than half
// the frame (the source's aspect ratio is too different from the target's
// to look right), that's flagged as needing manual adjustment instead of
// silently shipping a mostly-blank poster.
...
// that range or too small/large - most posters never need one. When one is
