...
import { validateAssetForPlatform } from "./socialPlatformRules.ts";

Deno.test("a well-formed poster passes Facebook feed validation", () => {
...
Deno.test("a well-formed square poster passes Instagram feed validation", () => {
...
Deno.test("LinkedIn is architecturally present but reports unsupported (Phase 2), never silently accepted as publishable", () => {
...
Deno.test("a poster can fail Instagram but pass Facebook independently (canonical, per-platform rules, not one shared guess)", () => {
  // 1.91:1 landscape passes Facebook's wider ceiling but fails Instagram's tighter one only if outside its range - use a case that differs
  ...
  assertEquals(fb.valid, false); // 3.2 ratio exceeds Facebook's 1.91 ceiling too, but for a different reason set - just confirm independence below
