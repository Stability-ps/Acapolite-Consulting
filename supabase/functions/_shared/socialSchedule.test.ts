...
Deno.test("3-day cadence: 4 posters land exactly 3 local days apart", () => {
...
Deno.test("every slot keeps the same local time-of-day as the campaign start", () => {
...
Deno.test("18 posters at 3-day cadence produce 18 strictly increasing, correctly spaced dates", () => {
...
Deno.test("excluding a specific date shifts only that slot forward, without cascading into later spacing", () => {
  const start = zonedPartsToUtc({ year: 2026, month: 9, day: 1, hour: 9, minute: 0, second: 0 }, JHB);
  // Poster index 1 would naturally land on 2026-09-04; exclude it.
...
Deno.test("DST safety: a US Eastern-time campaign stays at the same local hour across the spring-forward transition", () => {
