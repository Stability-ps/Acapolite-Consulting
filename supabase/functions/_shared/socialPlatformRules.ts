// Single canonical source of platform posting requirements. Every place that
// needs to know "is this image OK for platform X" (asset upload validation,
// campaign-item validation before scheduling, the publish worker's final
...
export function getPlatformRules(key: SocialPlatformKey): PlatformImageRules {
  const rules = SOCIAL_PLATFORM_RULES[key];
  if (!rules) throw new Error(`Unknown platform key: ${key}`);
  return rules;
}

// Maps the social_accounts/social_platform enum value ("facebook",
// "instagram", "linkedin") to its validation-rules key ("facebook_feed",
// etc). Kept in one place so the activation preview and the publish worker
// can never drift apart on which ruleset applies to which account platform.
export function platformKeyForAccountPlatform(platform: string): SocialPlatformKey {
  ...
  throw new Error(`Unknown account platform: ${platform}`);
}
