...
import { generateVariantsForAsset, parseGenerateVariantsRequest, type GenerateVariantsDeps, type MediaAssetRecord } from "./socialVariantGeneration.ts";
...
const OVERSIZED_FOR_INSTAGRAM: Omit<MediaAssetRecord, "id" | "storage_path"> = {
  mime_type: "image/jpeg", width_px: 1536, height_px: 1024, file_size_bytes: 500_000, created_by: "user-1",
};

Deno.test("REGRESSION: regenerating asset A never creates, updates, or removes anything for asset B", async () => {
  const assetA: MediaAssetRecord = { id: "asset-A", storage_path: "orig/A.jpg", ...OVERSIZED_FOR_INSTAGRAM };
  const assetB: MediaAssetRecord = { id: "asset-B", storage_path: "orig/B.jpg", ...OVERSIZED_FOR_INSTAGRAM };
...
Deno.test("an asset that already passes validation for both platforms needs no variant and writes nothing", async () => {
  const asset: MediaAssetRecord = { id: "asset-ok", storage_path: "orig/ok.jpg", mime_type: "image/jpeg", width_px: 1080, height_px: 1080, file_size_bytes: 200_000, created_by: "user-1" };
