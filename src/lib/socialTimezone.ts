// Frontend timezone helpers for the Social Media Scheduler. The DST-safe
// conversion algorithm mirrors supabase/functions/_shared/socialSchedule.ts
// (see socialSchedule.test.ts for the DST-safety proof) - it's duplicated
// here only because the edge function (Deno) and this frontend (browser)
// are separate runtimes with no shared import path.
...
export function formatInBusinessTimezone(date: Date | string, options: Intl.DateTimeFormatOptions = {}): string {
